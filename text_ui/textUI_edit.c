...
static WINDOW EditApplication = NULL;
...
static void ShowPosition(WINDOW wnd) {
	...
	{
		snprintf(status, 50, "%c %c Li:%d Co:%d",
		...
		snprintf(status, 50, "%c %4s  Line: %4d  Col: %3d ",
				(cfg.ReadOnlyMode) ? 'R' : (wnd->TextChanged ? '*' : ' '),
		...
		for (x = 0; x < 16; x++) {
			...
			if ((!content[i - 1])
                ...
                || (content[i-1] == CHANGECOLOR)
					...
					|| (content[i - 1] == ('\t' | 0x80)) || (content[i - 1] == ('\f' | 0x80))
#endif
			)
				content[i - 1] = '*';
			if (ascii_highlight == ch)
...
static int AsciitabKeyboardMsg(WINDOW wnd, PARAM p1) {
	switch ((int)p1) {
		case UP:
		case PGUP:
			ascii_highlight -= 16;
...
#endif
	if (ascii_highlight < 0)
		ascii_highlight += 256;
	if (ascii_highlight > 255)
		ascii_highlight -= 256;
	DisplayAsciitab(wnd);
	return TRUE;
}
